      ...
      <v-menu v-model="open" :close-on-content-click="true" transition="fade-transition" location="bottom start">
        <template #activator="{ props }">
          <v-btn v-bind="props" class="segment-button" size="small" variant="text" append-icon="mdi-chevron-down"
            role="combobox" aria-haspopup="listbox" :aria-expanded="String(open)" aria-controls="scene-listbox" block>
            <span class="segment-value">{{ selectedName || '—' }}</span>
          </v-btn>
        </template>
        <v-list id="scene-listbox" class="dropdown-list" density="compact"
...
function select(id) {
  // Close menu first, then emit on next frame so overlays can render
  open.value = false
