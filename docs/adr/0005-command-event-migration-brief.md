...
mjlab, with tests that fail under `Promise.all`. `tick`'s interval terms stay
fire-and-forget — it is synchronous because the step loop is, so there is nothing
to await into — noted rather than solved, since no play config has even one traced
interval term. Trading a free correctness
