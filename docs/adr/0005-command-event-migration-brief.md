...
| `height_scan` / `RayCastSensor` slots | **Python done, runtime pending** — a structured sensor (`.data` is a struct of ray hits, not a `sensordata` window) now contributes *one slot per field the term reads* rather than looking like one opaque blob. That blob is what made `height_scan` look untraceable and got it frozen as 187 constants. `height_scan` traces clean against mjlab (max\|Δ\|=0 over 6 stepped frames) and both Velocity-Rough groups fuse at their real widths (286, 235). Still missing browser-side: a raycast reader. `mj_ray`/`mj_multiRay` *are* in the WASM build and the sensor is fully describable — `GridPatternCfg(size=(1.6,1.0), resolution=0.1, direction=(0,0,-1))` → 187 baked local offsets, frame `robot/pelvis`, `ray_alignment="yaw"`, `max_distance=5.0`, `exclude_parent_body=True` — so what remains is emitting that descriptor and casting per frame. Until then a `{sensor, field}` slot reports unavailable *by name*, so the term holds its last value and says so, rather than reaching into `sensordata` where this sensor has no window |
