...
the same entity (`cartpole`), and Velocity's two write different kinds. What makes
that safe today is **disjointness, not ordering** — `onReset()` fires the terms
through `Promise.all`, so they are concurrent and the writes land in resolution
order, and it is correct only because each graph's baked joint indices touch a
different slice. (This section previously said sequential dispatch gave them dict
order for free; that was wrong about the code, and worth stating because the
distinction is the whole argument.) One fused graph emits *one* `joint_state` write
per entity, so the two contributions would have to be combined inside it — which
means establishing that disjointness at trace time and encoding it, rather than
relying on it implicitly. Not hypothetical: it lands on the first task in the set. Trading a free correctness
