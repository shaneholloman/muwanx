...
| RSI jitter reachable from the examples | **done (regression fix)** — reconciling the carved-out table turned up a silent loss this effort introduced. `TrackingCommand.ts` used to jitter the reference frame with `Math.random()`; that moved into a traced graph whose body needs mjlab's own `sample_uniform`/`quat_from_euler_xyz`, so it is registered author-side from `examples/mjlab/defaults/commands` rather than from `mjswan.command` (which keeps mjlab a soft dependency). **`g1_spinkick` never imported that module**, so it resolved to the plain built-in binding — no graph — and `buildResetJitter` returned `null` without a word, starting every episode from the unjittered reference frame where mjlab's play config asks for `joint_position_range=(-0.1, 0.1)`. Two fixes: the example imports the registration, and the built-in binding now carries a `reset_trace` that exists only to diagnose its own absence — it returns `None` either way, silently when the config jitters nothing (a task with every range cleared is missing nothing) and with a `RuntimeWarning` naming the ranges when it does. Verified against the real task config: without the import, warns and no graph; with it, the graph and no warning. Also fixed a comment in that example claiming the terminations were "declarative built-ins in `mjswan.envs.mdp.terminations`" — the DSL is gone and the four names it listed are not this task's (`anchor_pos`, `anchor_ori`, `ee_body_pos`) |
| §4 pre-allocate input tensors | **not adopted — measured decline.** ADR §4's session-hygiene note asks to "pre-allocate input tensors and write in place per frame rather than re-allocating", singling out `runtime.ts` as the offender. Measured on Velocity-Flat-G1 (5 slots + 2 native inputs, 99-wide group) with the real graph, the real `SlotReader` and a real ORT session, 2000 iterations after warm-up: **`compute()` costs 0.29 ms, which is 1.5% of a 20 ms control step.** Splitting it, the slot-read-and-wrap half is 0.20 ms (69%) and the ORT run 0.09 ms — but of that 0.20 ms, **allocation alone is 0.0067 ms**: 2.3% of `compute()`, 0.03% of the frame. The rest is *work* pre-allocation cannot remove — float64→float32 per element, an encoder-bias map lookup per joint, the `projected_gravity_b` quaternion math — because those convert the same elements whichever buffer they land in. The instruction targets noise. The named policy path is the same order: per frame it spreads a small object, allocates one one-element array for `time_step`, and wraps existing `Float32Array`s in `ort.Tensor` without copying. If this path is ever worth optimising, the measurement says the lever is the reader's per-element work (69%), not allocation (2.3%) — and at 1% of a frame against rendering and `decimation × mj_step`, it is not the frame-rate lever either. Caveat: measured under Node with ORT-Web's wasm backend, so the absolute numbers are indicative; the ratios would survive a several-fold shift either way |
| Event dispatch order matches mjlab | **done (correctness fix)** — `onReset()` and `startup()` fired their terms through `Promise.all`, so writes landed in *resolution* order. mjlab's `EventManager.apply` loops a mode's terms in config order, and every write is an assignment (`data.qpos[env_ids, q_slice] = position`), so two terms touching the same element resolve last-writer-wins **by config order**. Concurrent dispatch made that machine-dependent instead — invisible while writes are disjoint, which they are on every reference task, and nondeterministic the moment they overlap. Both now loop and await in order; tests use sessions that resolve *out* of config order and fail under `Promise.all`. Sequencing does not make terms compound, because their bodies read `default_*` rather than live state (mjlab's `reset_joints_by_offset` starts from `default_joint_pos`) — it only decides which value survives an overlap. Costs one reset's worth of serialized inference, on a rare frame. `tick`'s interval terms were left fire-and-forget on the grounds that `tick` had to be synchronous because the step loop was — which stopped being true (the loop awaits observation collection, inference and the reset chain, and `tick` is its last call), so they now loop and await in config order as well, with the same out-of-order-session test. All three modes match mjlab. Still no play config has a traced interval term, so that third one closed a hole rather than fixing observed breakage |
| §4 Event fusion per `mode` | **not adopted — measured decline** (see §4b). Traced event terms per mode across the reference tasks: `startup` has **zero** (it is all model-field descriptors, which carry no graph, plus native `encoder_bias`), no play config has a traced `interval` term, and `reset` has at most **two**. So fusion would reach one mode, two terms, on a call that happens at an episode boundary — one fewer `ort.run()` per reset — and Lift's single traced reset term is excluded anyway by the same rule that leaves a lone termination unfused. The cost is real but modest — Cartpole's two reset terms write the same entity's `joint_state`, so a fused graph would have to combine them, and mjlab's semantics settle the rule (assignment in config order, so last-writer-wins per element: a scatter when disjoint, a select-by-term-order when not). The decline rests on the payoff, not the cost. Establishing those semantics is what turned up the ordering fix below. Revisit if a task ever has several traced terms in a mode that fires often. |
...
| §4 Event dispatch wired into the loop | **done** — `EventManager.startup()`/`tick()` existed but nothing called them, so `mode="startup"`/`"interval"` terms loaded and then never fired. `startup()` now runs once at the end of `loadEnvironment` (after the model *and* policy exist, so its writes are not clobbered by the policy load's reset) and `tick(dt)` runs each control step beside `commandManager.update`. `onReset()` was left un-awaited on the strength of `resetSimulation()` being synchronous out through the engine API — but the step loop is not that caller, and it now awaits the whole reset chain; see the row below. The public verb still does not |
| **Reset chain ordering matches `_reset_idx`** | **done (correctness fix)** — mjlab's `_reset_idx` is explicitly order sensitive and, for the five managers mjswan keeps, runs `event(mode="reset")` → `observation.reset` → `action.reset` → `command.reset` (which *is* `_resample`), then the step's single `sim.forward()`, then `_update_command`, then `termination.reset`. mjswan's loop had all four of those points wrong in some way. **(a)** `onReset()` was fired un-awaited (`void`) with `commandManager.resetTerms()` on the next line — not merely a race: both sides write `qpos` (a reset event writes the perturbed initial state, `TrackingCommand.reset` writes the reference pose through `applyReferenceStateToSim`, synchronously), so the command's write landed *first* and the event's ORT-backed write overwrote it — mjlab's last-writer-wins-by-config-order resolved backwards, every time, on the tracking tasks. **(b)** `policyRunner.reset` (mjlab's observation + action reset in one call) ran *after* the command reset, purely because that was where the call sat; no consequence today since no reference command reads `last_action`, matched anyway because a divergence kept for no reason is one a future term inherits. **(c)** the resample ran on the far side of the forward — see the row below. Now one awaited sequence in `core/engine/resetChain.ts`, extracted because `runtime.ts` needs the MuJoCo WASM module to instantiate and therefore has no tests at all, and an ordering guarantee with no test is one `void` away from reverting. Six cases pin it; dropping the await fails five, reordering the policy reset past the commands fails one. The public `resetSimulation()` keeps its synchronous signature (ADR 0004) and voids the promise; the step loop awaits and catches, so one failing graph costs a reset rather than the run |
| **Reset-time resample runs before the forward** | **done (correctness fix)** — mjlab resamples on *both* sides of its single forward depending on the trigger: a timer expiry from `command_manager.compute(dt)` after it, a reset from `command_manager.reset(env_ids)` inside `_reset_idx` before it, with `_update_command` always after. mjswan matched the timer case and diverged on reset, because `OnnxCommand.reset` only raised `pendingResample` and left the whole graph to the post-forward `update()`. The **reads** that made divergent turn out to be harmless — no reference command's `_resample_command` touches a derived quantity (`UniformVelocityCommand` reads only its cfg ranges and the rng, `MotionCommand` reads motion-clip buffers `motion.body_pos_w[time_steps]` rather than `asset.data`, `LiftingCommand` reads `sample_uniform` and `env_origins`) — so the earlier worry about an RSI anchor seeing different state was misplaced. The **writes** are the problem, and §3b above has the detail. `OnnxCommand.reset` now runs the graph itself with `resample_mask = 1`, awaited by `CommandManager.resetTerms` (sequential, config order, for the same overlapping-write reason `onReset` is), which the reset chain awaits before the forward; the frame's later `update()` runs the same graph with `resample_mask = 0`, which is `_update_command` alone. That reproduces mjlab's split across the forward for one extra `ort.run()` on reset frames — **no second forward**, and ADR §3's "reset unifies to `resample_mask = true`" intact: a second call to the same graph, not a separate reset path |
| **`generated_commands` names bound at load** | **done (silent-failure fix)** — `CommandManager.getCommand` returns an empty `Float32Array` for a name it does not hold, which `conformToSize` then padded to the declared width: a renamed or typo'd command — or one whose `OnnxCommand` session was missing and got warn-and-skipped — reached the policy as a block of zeros at a fixed offset in its input vector, logged nowhere. mjlab asserts the lookup (`envs/mdp/observations.py`). The name is now bound once where the config is read: `termNames()` exposes the registered names and `assertCommandTermBound` throws from both observation readers' constructors, naming the miss and the available set. Matches how a missing observation *graph* is already handled (`PolicyRunner.buildOnnxObservation` throws, for the same reason — an observation slice reshapes the vector it feeds) and leaves the sanctioned way to express an undriven slot untouched (`native: "constant"`, whose value the build bakes). An absent `CommandManager` stays non-fatal: a host that runs no commands is not evidence of a wrong *name*. Also removed a dead guard — `FusedObservation`'s `?? new Float32Array(size)` could never fire, since `getCommand` returns an empty (truthy) array rather than `null`, so it read as a guard while guarding nothing |
| **`last_action(action_name=…)` reads its own slice** | **done (latent bug)** — mjlab's `last_action` with a term name is `get_term(name).raw_action`, that term's slice of the policy output. The build emitted `action_name` from the start and nothing in the runtime read it, so a named term got the whole action vector and `conformToSize` truncated it to the declared width *from the front*: the right width, the wrong term's numbers. The build now resolves the offset (`action_term_offset` walks `action_term_dim` in config order, exactly as `ActionManager.process_action` splits the vector) and emits `action_offset`; both readers narrow through `sliceStoredActions`. Unresolvable names raise, deliberately outside the best-effort `size` probe around them — that probe exists so a browser-only command can be emitted without a width, and swallowing an offset failure the same way would restore the whole-vector read. **Still unreachable from a buildable task:** every reference task declares one action term, where the slice and the vector coincide, which is why nothing caught it; the two-term shape is covered by unit tests on both sides only |
...
| A term with no browser implementation fails the build, events included | **done** — the rule held for observations and (fused) terminations only. A `*Binding` without `ts_src` named a class the browser does not have — mjswan ships no built-in TS term classes, so every such binding resolved to nothing — and an untraceable **event** was emitted as `{native: true, reason}`, which the runtime skips silently: a configured reset randomization simply did not happen, with nothing said at build time or in the browser. Both now raise, naming the two ways out (a trace-friendly `register_*` replacement, or a `ts_src` TS class). The event path keeps three stated exemptions rather than a catch-all, each because there is provably nothing to write: `randomize_terrain` (one baked terrain, one origin), `encoder_bias` (the runtime applies it from the policy config), and a root-state write onto a **fixed-base** entity (cannot move it in mjlab either — mjlab's manipulation tasks configure `reset_base` on their arms regardless). Measured across all 10 buildable registry tasks: every one still serializes, and `cube_color` (`geom_rgba`) moved *out* of the fallback into a described model-field DR, so the Rgb variant's colour randomization is no longer dropped. `unsupported_reason` is deleted from the three MDP bindings — a marker whose whole job was "accepted then fails at build" is redundant once the absence of an implementation fails at build. `TerminationManager` throws on an unknown term name like the observation and command registries already did, instead of warning and continuing without a reset condition |
...
| g1_spinkick | Tracking-Flat-…-No-State-Estimation | `TrackingCommand` (native motion lookup) + traced RSI graph | **design settled; build needs the W&B motion artifact.** The command seam is no longer open: the clip lookup stays native and the reference-state-initialization jitter is traced (`motion_rsi_offset`, verified against this task's own play config). The task config loads offline — commands `['motion']`, terminations `['time_out', 'anchor_pos', 'anchor_ori', 'ee_body_pos']` — and env construction is what fails without the artifact (`FileNotFoundError` on the empty clip path) |
| musclemimic | myoMimicFullbody-v0 | `TrackingCommand`-shaped (Mimic) | **carved out** (private-repo dep) |
| unitree_rl | Unitree-G1-Tracking-… | `TrackingCommand`-shaped | **carved out** — `main.py` imports the author's own `src.tasks`, which is not in this repo, so it is a template for a user's repo rather than a runnable example here |
...
- **`entity_write`:** `_resample_command` samples the cube's own spawn pose/velocity
  ...
  **Timing:** mjlab runs `_resample_command` from `command_manager.reset(env_ids)` inside
  `_reset_idx`, *before* the step's single `sim.forward()` — so that forward is what
  publishes the write. This brief first called the browser's post-`forward()` write "the
  accepted one-substep lag" and closed the question. Wrong on the conclusion, right that a
  second `forward()` is not the answer: a write with no forward after it is not a lagging
  write, it is an **unpublished** one. `qpos` moves and `xpos` does not, so an observation
  reading `root_link_pos_w` (which the slot reader serves from `mjData.xpos`) sees a pose
  from before the cube was placed — and mjswan exists to reproduce the env the policy was
  trained in, where that forward had already run. Resolved by running the resample where
  mjlab runs it, not by adding a forward: see the reset-chain row in §Status.
...
*The cost, and it is smaller than this section first claimed.* Cartpole's
`reset_slider` and `reset_hinge` both write `joint_state` on the same entity, so a
fused graph — which emits one write per entity per kind — would have to combine two
contributions into one tensor. That is not an open question: mjlab's semantics are
**assignment in config order**, so the merge rule is last-writer-wins per element,
which is a scatter for the disjoint case and a select-by-term-order for an
overlapping one. Both are expressible in a graph. So the decline rests on the
payoff, not on this; an earlier draft of this paragraph overstated it as a
correctness problem.

*What the investigation did turn up.* Establishing mjlab's semantics showed
`onReset()` was firing its terms through `Promise.all` — concurrently, so writes
landed in *resolution* order where mjlab loops in config order. Harmless while
writes are disjoint (every reference task), nondeterministic the moment they
overlap. `onReset` and `startup` now loop and await in config order, matching
mjlab, with tests that fail under `Promise.all`. `tick`'s interval terms were left
fire-and-forget at the time — `tick` was synchronous because the step loop was, so
there was nothing to await into. That reason expired: the loop now awaits
observation collection, inference and the reset chain, and `tick` is its last call,
so the interval mode loops and awaits in config order too. All three modes match
mjlab. No play config has a traced interval term yet, so that third one closed a
hole rather than fixing observed breakage. Trading a free correctness
...
(`tests/test_onnx_parity.py::test_push_robot_matches_mjlab`):
...
13. **`UniformVelocityCommand` is blocked — the first term to hit §9's escape hatch
    ...
    *Confirmed empirically* at the time: the
...
14. **`UniformVelocityCommand`'s path — resolved by an examples-side trace-friendly
    ...
    different RNG structures anyway). *Validated* on Go1-Velocity-Flat — now asserted by
    `tests/test_onnx_command_parity.py`: the overridden `twist` traces to ONNX and holds parity —
...
16. **`OnnxCommand` `policy.json` config output — done.** `mjswan.compile.serialize`
    ...
    from the real velocity-override trace (`tests/test_onnx_command_parity.py`).
...
Two of the three held; the four rows above are what did not. The reset chain now runs
mjlab's `_reset_idx` order end to end, `generated_commands` and
`last_action(action_name=…)` resolve their names at load instead of degrading, and the
reset-time resample happens where mjlab's does. Two findings are worth carrying rather
than filing as done:

- **The audit's three relationships are the ones `rolloutParity` stubs.** Its fake runner
  supplies `getLastActions` and `getCommand` from the fixture, and `getCommand` ignores its
  name argument — so the harness proves the *graphs* consume `prev_action`/`command`
  correctly and proves nothing about `PolicyRunner.setLastActions`, the name lookup, or the
  reset order. Every fix above is unit-tested; none is covered end to end.
- **`loadPolicyConfig` swallows the throws these fixes rely on.** The whole policy load is
  wrapped in `try { … } catch { console.warn(…) }`, so a binding failure leaves
  `policyRunner` null with one warning rather than surfacing. Better than a silently wrong
  input vector, and consistent with how the pre-existing `buildOnnxObservation` throw is
  already treated — but the swallow is its own question, untouched here because changing it
  reaches the engine's `loadPolicy` contract.
