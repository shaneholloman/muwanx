...
| §3 ONNX **termination** runtime (TS) | **not started** — the build emits `{onnx}` / `{native: "elapsed_s >= episode_length_s"}` termination entries that `TerminationManager.ts` does not yet consume |
| §4 byte delivery for observation/termination sessions | **not started** — `PolicyRunnerOptions` now takes `onnxSessions`/`readOnnxSlot`, but nothing populates them yet (same deferred seam as commands/events) |
