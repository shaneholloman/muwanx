...
| §2 bit-for-bit replay itself | **not met — mechanism is necessary but not sufficient** (finding). Exposing the seed does not deliver ADR §2's guarantee, because every traced term draws from **one shared stream** while `OnnxCommand.update`, `OnnxEvent.fire`, `OnnxTermination.evaluate` and `FusedTermination.kick` each *skip* their run when a previous one is still in flight ("skip, never queue" — the async boundary §8 chose over blocking the loop). Whether a frame skips depends on how long ORT took against the frame budget, i.e. on wall-clock, i.e. on the machine. A skipped frame therefore consumes one fewer draw and **shifts the stream for every other term from that point on** — not a drift but a hard divergence. Demonstrated rather than asserted: `core/__tests__/rng.test.ts` now pins the coupling (one consumer skipping frame 2 changes an unrelated consumer's frames 2..5) and sketches the fix as a second test. **The fix is step-addressed draws**: derive `rand` from `(seed, term, step)` with a counter-based mix instead of pulling from a sequential stream, so frame *N* gets the same numbers whether or not anything else ran. Bounded but not trivial — a step index has to reach the command/event/termination update calls — and it changes every drawn value, so it wants doing deliberately rather than folded into this row. Until then: reproducible in practice (these graphs settle in well under a frame, so skips are rare), not guaranteed. |
...
- [ ] A recorded session replays bit-for-bit from its persisted PRNG seed (ADR
      acceptance criterion 2). The seed is now choosable and readable through the public
      API, which it was not; the guarantee still needs step-addressed draws, because one
      shared stream plus skip-if-in-flight makes the draw *count* wall-clock-dependent.
      See the two §2 rows above.
