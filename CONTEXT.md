...
- `serialize.py`: a traced command's `policy.json` entry + `COMMAND_JSON_SCHEMA`. One generic browser-side `OnnxCommand` handler interprets every command from this data, so the entry declares state fields (names, shapes, **and initial values**), `rand_dim`, dynamic reads, and any `entity_write`.
